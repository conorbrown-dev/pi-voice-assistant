...
Whisper is the default speech recognizer. It records one utterance after detecting speech and transcribes it locally with `whisper.cpp`; start with `--sample-rate 44100` for the USB microphone tested here. To retain the previous Vosk listener, download and unpack `vosk-model-en-us-0.22-lgraph`, set `VOSK_MODEL_PATH` to its directory, and start with `--stt vosk`.
